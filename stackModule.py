...
class Stack:
    # Stack 클래스가 생성될 때 크기를 넘겨받으면 넘겨받은 크기만큼의 기억공간을 가지는 스택을 생성하고 스택의 크기를 
    ...
    def __init__(self, size=5):
        ...
        # top, sp(stack pointer) => 스택에 몇 개의 데이터가 저장되어 있나 기억한다.
        # 데이터가 입력되면 1증가. 스텡에서 데이터가 출력되면 1감소 => 스택의 크기와 비교해서 overflow, underflow를 처리한다.
        self.top = 0
    ...
    def push(self, data):
        ...
        if data not in self.stack: # 스택에 저장하려는 데이터가 스택에 존재하지 않는가?
            ...
            # 스택의 크기가(self.size) 5일 때 스택으로 사용할 리스트의 인덱스(self.top)는 0,1,2,3,4만 사용할 수 있다.
            if self.top < self.size:
                ...
                # overflow가 발생되면 가득찼다는 메시지를 출력한다.
    ...
    def pop(self):
        ...
        if self.top <= 0:
            ...
            data = self.stack.pop()
            # 스택에 저장된 데이터가 출력 되었으므로 top을 1감소 시킨다.
            self.top -= 1
